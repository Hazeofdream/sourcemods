...
#define PLUGIN_AUTHOR		"NoroHime, Haze_of_dream"
...
ConVar cAvoidTankCrescendo;	bool bAvoidTankCrescendo;
...
public void OnPluginStart() {
	...
	cAvoidTankCrescendo = CreateConVar(PLUGIN_NAME ... "_avoid_events", "1", "1=don't trigger timed hordes while a Tank is alive or a Crescendo/panic event is active, 0=disabled", FCVAR_NOTIFY);
	...
	cAvoidTankCrescendo.AddChangeHook(OnConVarChanged);
	...
	// Crescendo events (scripted "panic" mob events, e.g. alarms/gas cans) start
	// with create_panic_event and end with panic_event_finished.
	HookEvent("create_panic_event", OnCrescendoStart);
	HookEvent("panic_event_finished", OnCrescendoEnd);
...
void ApplyCvars() {
	...
	bAvoidTankCrescendo = cAvoidTankCrescendo.BoolValue;
}
...
void OnCrescendoStart(Event event, const char[] name, bool dontBroadcast) {
	bInCrescendoEvent = true;
}

void OnCrescendoEnd(Event event, const char[] name, bool dontBroadcast) {
	bInCrescendoEvent = false;
}
...
bool TriggerTimedHordes(bool makeforward = true) {
	...
	if (bAvoidTankCrescendo && (bInCrescendoEvent || IsTankOnGround()))
		return false;
